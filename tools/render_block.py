...
B = f'''<div class="eyebrow" style="margin:18px 0 8px">Machine extraction from the calendar photographs &mdash; internal only</div>
  ...
  <p class="note"><b>What an operation sensor would size, and nothing else.</b> <b>None of this is available on calendar evidence</b> &mdash; the methodology permits a value above <span data-param="do_cap"></span> days only on measured sensor data, so the arithmetic exists to size the instrumentation decision and for no other purpose. Emission reductions are <b><span data-param="er_maro"></span> tCO<sub>2</sub>e per community water supply per year</b> in the VPA-DD at <b><span data-param="do_cap"></span></b> days, and <span class="mono">DO<sub>p,y</sub></span> enters the volume calculation linearly. The span from <span data-param="do_cap"></span> days to a full common year is <b>+<span data-fig="(100*(365/PARAMS.do_cap.v-1)).toFixed(2)"></span>%</b>, or <b>+<span data-fig="(PARAMS.er_maro.v*efbMaro(PARAMS.fnrb_maroantsetra_applied.v)/efbMaro(PARAMS.fnrb_mofuss_maroantsetra.v)*(365/PARAMS.do_cap.v-1)).toFixed(2)"></span> tCO<sub>2</sub>e per point per year</b> on the Maroantsetra figure with its fNRB applied at <span data-fig="Math.round(PARAMS.fnrb_maroantsetra_applied.v*100)"></span>%. Fort-Dauphin carries the higher baseline emission factor (<b><span data-param="efb_anosy"></span></b> against <b><span data-param="efb_maroantsetra"></span></b> tCO<sub>2</sub>e/L as registered), so that span is wider there. With the Maroantsetra fNRB applied at <span data-fig="Math.round(PARAMS.fnrb_maroantsetra_applied.v*100)"></span>% rather than the registered <span data-fig="Math.round(PARAMS.fnrb_mofuss_maroantsetra.v*100)"></span>% (both MoFuSS-derived; James Walker, 21 Sep 2026), its emission factor becomes <b><span data-fig="efbMaro(PARAMS.fnrb_maroantsetra_applied.v).toFixed(6)"></span></b> tCO<sub>2</sub>e/L and its emission reductions <b><span data-fig="(PARAMS.er_maro.v*efbMaro(PARAMS.fnrb_maroantsetra_applied.v)/efbMaro(PARAMS.fnrb_mofuss_maroantsetra.v)).toFixed(1)"></span></b> tCO<sub>2</sub>e per community water supply per year, from the registered <span data-param="er_maro"></span>. A sensor would not deliver a full year either: it would deliver whatever it measures, and the metering programme is fleet-wide &mdash; a logger on every metered pump &mdash; so the comparison is the whole programme against the whole fleet, not a per-unit trade. <b>Whether it is worth the instrumentation is a decision for the Head of Carbon, not a conclusion of this page.</b></p>
