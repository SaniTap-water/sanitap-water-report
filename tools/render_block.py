...
B = f'''<div class="eyebrow" style="margin:18px 0 8px">Machine extraction from the calendar photographs &mdash; internal only</div>
  ...
  <p class="note"><b>Two gates, two questions.</b> Whether a sheet can be read at all is decided by fitting twelve month columns to the printed rules; whether individual days can be called on it is decided separately, by registering the day rows to the same rules. <b>{n(fig["images_readable"])}</b> photographs pass the first, <b>{n(fig["images_with_day_calls"])}</b> the second. <b>Coverage figures use the first; every day-level figure uses only the second.</b></p>

  <p class="note"><b>Indicative uptime, backed out.</b> Subtracting the impossible-cell false-positive floor from the marked rate on observed cells gives an <b>implied true marked rate</b> of {fig["observed_marked_pct"]:.2f}% &minus; {fig["probe_marked_pct"]:.2f}% = <b>{fig["implied_true_marked_pct"]:.2f}%</b> <sup>&dagger;</sup>, which is <b>{fig["implied_days_not_operational"]} days not operational</b> in a 365-day year and an implied uptime of <b>{fig["implied_uptime_days"]} days</b> <sup>&dagger;</sup>. <b>This is not a figure we can apply.</b> It sits above the <b>347</b>-day cap, and the calendars are a manual log, so the registered basis holds it at 347 regardless. What it does is test the condition for <i>keeping</i> 347: downtime must not exceed <b>18</b> days per point-year, and <b>{fig["implied_days_not_operational"]}</b> days is well inside that. <b>The test bounds over-counting only</b> &mdash; it says nothing about faint marks the software missed, which would push the other way, and nothing here can measure that until a human has read the same sheets. <span class="muted"><a href="#act-transcription-round">Run the transcription validation round.</a></span></p>
  ...
  <p class="note"><b>The year has to be read off the sheet, and on <b>{n(fig["sheets_not_dated"])}</b> readable photographs it cannot be.</b> Of <b>{n(fig["images_readable"])}</b> readable photographs the printed year could be read on <b>{n(fig["sheets_dated_from_the_sheet"])}</b>. <b>Sheets whose year could not be read are excluded from every rate rather than assigned a year</b> &mdash; an undated sheet cannot be attached to a monitoring year, so it evidences nothing however well it is read. By year: {", ".join(f"<b>{v}</b> for {k}" for k, v in sorted(sy.items()))}. <span class="muted"><a href="#act-undatable">Quantify the water points behind the undatable sheets.</a></span></p>
  ...
  <p class="note"><b>What an operation sensor would size, and nothing else.</b> <b>None of this is available on calendar evidence</b> &mdash; the methodology permits a value above 347 days only on measured sensor data, so the arithmetic exists to size the instrumentation decision and for no other purpose. Emission reductions are <b>27.9 tCO<sub>2</sub>e per community water supply per year</b> in the VPA-DD at <b>347</b> days, and <span class="mono">DO<sub>p,y</sub></span> enters the volume calculation linearly. The span from 347 to 365 days is <b>+5.19%</b>, or <b>+<span data-fig="(PARAMS.er_maro.v*(365/PARAMS.do_cap.v-1)).toFixed(2)"></span> tCO<sub>2</sub>e per point per year</b> on the Maroantsetra figure. Fort-Dauphin carries the higher baseline emission factor (<b>0.00027</b> against <b>0.00018</b> tCO<sub>2</sub>e/L), so that span is wider there. A sensor would not deliver 365 either: it would deliver whatever it measures, and the metering programme is fleet-wide &mdash; a logger on every metered pump &mdash; so the comparison is the whole programme against the whole fleet, not a per-unit trade. <b>Whether it is worth the instrumentation is a decision for the Head of Carbon, not a conclusion of this page.</b></p>
